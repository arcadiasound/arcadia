...
export const BuyAssetDialog = ({ open, onClose, address, track }: BuyAssetDialogProps) => {
  ...
  const isAssetOwner = ucmAsset && address && address in ucmAsset.state.balances ? true : false;
    ...
    >
      ...
      <DialogContent>
        {buyAssetMutation.isSuccess ? (
          <Flex css={{ my: "$5", height: "100%" }} direction="column" align="center" gap="5">
            <Box
              css={{
                color: "$slate12",
                "& svg": {
            ...
            <Typography as="h2" size="5" css={{ textAlign: "center" }}>
              Congratulations! <br /> {isAssetOwner ? "You own more" : "You're an owner"} of{" "}
              <Typography as="span" size="5" contrast="hi">
                {track.title}
              </Typography>
            </Typography>
            <Typography size="1" css={{ maxWidth: "40ch", textAlign: "center" }}>
          ...
          <>
            ...
            <Box as="form" onSubmit={formik.handleSubmit}>
              ...
              >
                ...
                >
                  Amount
                  <Typography as="span" size="1" css={{ position: "absolute", right: 0 }}>
                    (Max. {quantityForSale})
                  </Typography>
                </Typography>
                <TextField
                ...
                {formik.errors.spend ? (
                  ...
                  <FormHelperText css={{ right: 0 }}>
                    Price: {(getTotalSalePrice(formik.values.spend) / 1e6).toFixed(3)} U
                  </FormHelperText>
                )}
              </FormRow>
              <Typography
                    ...
                    >
                      ...
                      {Math.round(
                        getOwnershipPercentage(
                          isAssetOwner
                            ? ucmAsset.state.balances[address] + formik.values.spend
                            : formik.values.spend,
              ...
              {buyAssetMutation.isError && buyAssetMutation.error instanceof Error && (
                <Typography css={{ color: "$red11", mb: "$3" }} size="1">
                  {buyAssetMutation.error.message}
                </Typography>
              )}
              <Button
                disabled={formik.isSubmitting || buyAssetMutation.isLoading || !hasEnoughFunds()}
                variant="solid"
                css={{ width: "100%", mt: "$3" }}
                type="submit"
              >
