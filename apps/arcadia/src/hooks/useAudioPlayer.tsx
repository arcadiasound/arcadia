...
const AudioPlayerProvider = ({ children }: AudioPlayerProviderProps) => {
  ...
  const setAudioRef = () => dispatch({ type: "SET_AUDIO_ELEMENT", payload: audioRef });
  ...
  const setNextTrackId = (indexMatcher: number) => {
    const nextTrack = state.tracklist.find((track, index) => index === indexMatcher);
  ...
  const toggleShuffle = () => {
    ...
    if (!state.shuffle) {
      ...
      const shuffledTracklist = shuffleTracklist(state.tracklist, state.currentTrackIndex);
  ...
  const shuffleTracklist = (tracklist: Tracklist, currentTrackIndex: number): Tracklist => {
